...
import { ExternalLink, Eye, Heart, Calendar, ArrowLeft, BarChart3, List, Grid3x3, Share2, Copy, Check } from 'lucide-react';
...
export default function DeckDetailPage({ params }: PageProps) {
    ...
    const moxfieldIcon = 'https://zrcawujydeevzdhlwpin.supabase.co/storage/v1/object/public/media/moxfield.png';
    ...
    const avgCMC = cards.length > 0
        ? (cards.reduce((sum, dc) => sum + (dc.cards?.cmc || 0) * dc.quantity, 0) / totalCards).toFixed(2)
        : '0';
                            ...
                            <div className="relative bg-gradient-to-br from-primary/20 via-primary/10 to-transparent p-8 border-b border-border">
                                ...
                                <div className="relative">
                                    ...
                                    {deck.color_identity && deck.color_identity.length > 0 && (
                                        <div className="inline-flex items-center gap-1.5 bg-background/80 backdrop-blur-sm rounded-full px-4 py-2 mb-4 border border-border shadow-lg">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {deck.commanders?.map((cmd, idx) => (
                                            <span
                                                key={idx}
                                                className="inline-flex items-center px-4 py-2 rounded-full bg-primary/15 text-primary border border-primary/30 font-semibold text-sm hover:bg-primary/25 transition-all shadow-sm"
                                            >
                                                {cmd}
                                            </span>
                                        ))}
                            ...
                            <div className="border-b border-border bg-accent/30">
                                <div className="flex items-center px-6">
                                    <button
                                        ...
                                        className={`flex items-center gap-2 px-6 py-4 font-semibold transition-all relative ${activeTab === 'list'
                                            ? 'text-primary'
                                    ...
                                    <button
                                        ...
                                        className={`flex items-center gap-2 px-6 py-4 font-semibold transition-all relative ${activeTab === 'visual'
                                            ? 'text-primary'
                                            : 'text-muted-foreground hover:text-foreground'
                                            }`}
                                    >
                                    ...
                                    <button
                                        ...
                                        className={`flex items-center gap-2 px-6 py-4 font-semibold transition-all relative ${activeTab === 'stats'
                                            ? 'text-primary'
                                            : 'text-muted-foreground hover:text-foreground'
                                            }`}
                                    >
                                ...
                                {isLoading ? (
                                    ...
                                    <>
                                        ...
                                        <div className="mb-6 pb-4 border-b border-border">
                                            <div className="flex items-center justify-between mb-3">
                                                ...
                                                {selectedType && (
                                                    <div className="flex items-center gap-2 text-xs text-muted-foreground">
                                                        <span>
                                                            Showing {cards.filter(dc => dc.cards?.type_line?.includes(selectedType)).reduce((sum, dc) => sum + dc.quantity, 0)} of {cards.reduce((sum, dc) => sum + dc.quantity, 0)} cards
                                                        </span>
                                                        <button
                                            ...
                                            <div className="flex flex-wrap items-center gap-2">
                                                <button
                                                    onClick={() => setSelectedType(null)}
                                                    className={`px-3 py-1.5 rounded-lg border transition-all ${selectedType === null
                                                ...
                                                {['Creature', 'Instant', 'Sorcery', 'Artifact', 'Enchantment', 'Planeswalker', 'Land'].map((type) => {
                                                    ...
                                                    return (
                                                        <button
                                                            ...
                                                            className={`group flex items-center gap-2 px-3 py-1.5 rounded-lg border transition-all shadow-sm ${isActive
                                                                ? 'bg-primary text-primary-foreground border-primary shadow-md scale-105'
                                                                : 'bg-accent/50 hover:bg-primary/20 border-border hover:border-primary hover:shadow-md'
                                                                }`}
                                                        >
                                                            <span className={`text-sm font-bold transition-colors ${isActive ? 'text-primary-foreground' : 'text-foreground group-hover:text-primary'
                                                                }`}>
                                                                {type}s
                                                            </span>
                                                            <span className={`text-xs font-bold transition-colors px-2 py-0.5 rounded-full ${isActive
                                    ...
                                    <div className="space-y-8">
                                        ...
                                        <div>
                                            ...
                                            <TypeDistribution deckCards={cards} />
                                        </div>
                                    </div>
                                ) : (
