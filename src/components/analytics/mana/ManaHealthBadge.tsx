...
export function ManaHealthBadge({ healthScore }: ManaHealthBadgeProps) {
  const gradeColor = 
    healthScore.grade.startsWith('A') ? 'bg-green-500 hover:bg-green-600' :
    healthScore.grade.startsWith('B') ? 'bg-blue-500 hover:bg-blue-600' :
    healthScore.grade.startsWith('C') ? 'bg-yellow-500 hover:bg-yellow-600' :
      ...
      <Tooltip>
        <TooltipTrigger>
          <Badge className={gradeColor}>
            Mana: {healthScore.grade.split(' ')[0]}
          </Badge>
        </TooltipTrigger>
        <TooltipContent>
          <div className="space-y-1 text-xs">
            <div>Score: {healthScore.overall_score}/100</div>
            <div>{healthScore.total_lands} lands • {healthScore.unique_mana_sources} sources</div>
