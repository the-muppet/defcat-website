...
import { createManaMask, ManaSymbol } from "../utility/svg-mask";
  ...
  const toggleSwitchTheme = async () => {
    ...
    if (
      ...
      log('Animation config:', {
        x, y, finalMaskSize, duration, isHighResolution
      });

      const maskFunction = (animationType === ThemeAnimationType.MANA)
    ...
    if (animationType === ThemeAnimationType.CIRCLE || animationType === ThemeAnimationType.QR_SCAN) {
